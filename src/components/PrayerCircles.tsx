...
// components/PrayerCircles.js
import {timeHelper, getTodaysMonth } from "/utils/timeHelper.tsx"
  ...
  return (
    <div className="relative w-full h-96 max-h-500 my-4">
      ...
      {prayers.map((prayer) => (
        <div 
          key={prayer.name}
          className={`prayer-circle absolute w-48 h-44 rounded-full border-2 border-amber-200 backdrop-blur-lg shadow-lg shadow-amber-500/50 flex flex-col justify-center items-center z-10 transition-all duration-400 hover:scale-125 hover:shadow-amber-500/80 hover:z-20 ${prayer.position}`}
        >
